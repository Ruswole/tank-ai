...
**If creation fails due to a database limit**, list the user's existing projects and present them as an interactive menu. Do not delete the selected project immediately after the menu choice. First, display the selected project ID and the database ID that will be affected, and clearly state that the selected project and all of its databases and connections will also be deleted. Then require a separate explicit affirmative confirmation before invoking the deletion step. Only after the user confirms should you delete the selected project and retry creation. If the user declines, or the intended project is ambiguous, abort the recovery flow and leave the project unchanged.
...
2. Update `.env` by replacing the existing `DATABASE_URL` value in place, not by appending a second `DATABASE_URL` entry. Preserve all unrelated `.env` entries exactly as they are. If the file already contains a `DATABASE_URL` and the intended value is ambiguous or the user has multiple candidate values, ask for confirmation before overwriting it. If the key is missing, append a single `DATABASE_URL` line:
...
5. If `prisma/schema.prisma` does not exist, run `./node_modules/.bin/prisma init` to scaffold the project. This creates both `prisma/schema.prisma` and `prisma.config.ts`.
...
Only use `./node_modules/.bin/prisma db push` if the user explicitly asks for prototyping-only mode (no migration history). In that case, follow it with `./node_modules/.bin/prisma generate`.
...
> Before creating the script, verify the Prisma generator output in `prisma/schema.prisma`. In this project it is configured as `output = "../app/generated/prisma"`, so the client import must match `./app/generated/prisma/client.js`.
...
Run it with the repo-local, pinned runner:

```bash
./node_modules/.bin/tsx test-connection.ts
```

**Prisma 7 client instantiation rules:**

- Import from the configured generator output, e.g. `./app/generated/prisma/client.js` for this project (not `./generated/prisma`)
...
- **Prisma Studio (CLI):** `./node_modules/.bin/prisma studio` — opens a visual data browser locally
