...
Use Clerk's `dark` theme from `@clark/ui/themes` as the base.

Override appearance variables using the app's existing CSS variables. Do not hardcode colors.

## Sign-in and Sign-up pages:

- large screens: simple two-panel level
- left: compact logo, tagline, short text-only feature list
- right: centered Clerk form
- small screensL form only
- no gardients
...
- authenicated users redirect to `/editor`
...
Keep Clerk's default user menu and profile flows intact. Do not rebuild or heavliy customie Clerk internals.
...
- `ClerkProvder` warps the root layout
